...
import static com.example.jiangshujing.localpush.Contants.*;
...
//        NotificationManager manager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
        ...
        for (Integer key0 : keySet) {
            ...
            Log.i(TAG, "notifyObjectMap key" + key0);
            NotifyObject obj = notifyObjectMap.get(key0);
            Log.i(TAG, "notifyObjectMap value" + obj.toString());
            if (obj == null) {
                break;
            }
            ...
            if (obj.times.size() <= 0) {
                if (obj.noticeTime > 0) {
                    try {
                        ...
                        AlarmTimerUtil.setAlarmTimer(context, ++count, obj.noticeTime, TIMER_ACTION, map);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
